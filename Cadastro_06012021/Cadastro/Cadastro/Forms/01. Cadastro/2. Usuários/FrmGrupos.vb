...
Public Class FrmGrupos
    ...
    Private Sub Consultar()
        Me.dgvConsulta.DataSource = Banco.List("SELECT CODGRUPO, DESCRICAO FROM " & Banco.BancoNVOCC & "TB_SYS_GRUPO_USER ORDER BY DESCRICAO")
    ...
    Private Sub btnSalvar_Click(sender As System.Object, e As System.EventArgs) Handles btnSalvar.Click
        ...
        If txtCodigo.Text = String.Empty Then
            ...
            Try
                ...
                Codigo = Banco.ExecuteScalar("SELECT MAX(CODGRUPO) + 1 FROM " & Banco.BancoNVOCC & "TB_SYS_GRUPO_USER")
                If Codigo <> String.Empty Then
                    If Convert.ToInt32(Codigo) > 0 Then
                        If Banco.Execute("INSERT INTO " & Banco.BancoNVOCC & "TB_SYS_GRUPO_USER (CODGRUPO,DESCRICAO) VALUES ('" & Codigo & "','" & Me.txtDescricao.Text & "')") Then
                            Consultar()
                            Mensagens(Me, 1)
                        Else
                            Mensagens(Me, 4)
        ...
        Else
            ...
            Try
                If Banco.Execute("UPDATE " & Banco.BancoNVOCC & "TB_SYS_GRUPO_USER SET DESCRICAO = '" & Me.txtDescricao.Text & "' WHERE CODGRUPO = " & Me.txtCodigo.Text) Then
                    Consultar()
                    Mensagens(Me, 2)
                Else
                    Mensagens(Me, 5)
    ...
    Private Sub btnExcluir_Click(sender As System.Object, e As System.EventArgs) Handles btnExcluir.Click
        ...
        If Convert.ToInt32(Banco.ExecuteScalar("SELECT COUNT (*) FROM " & Banco.BancoNVOCC & "TB_SYS_GRP_PERMISSOES WHERE CODGRUPO = " & txtCodigo.Text)) > 0 Then
        ...
        If Not String.IsNullOrEmpty(txtCodigo.Text) Then
            If MessageBox.Show("Deseja realmente excluir o registro selecionado?", Me.Text, MessageBoxButtons.YesNo, MessageBoxIcon.Question) = Windows.Forms.DialogResult.Yes Then
                If Banco.Execute("DELETE FROM " & Banco.BancoNVOCC & "TB_SYS_GRUPO_USER WHERE CODGRUPO = " & txtCodigo.Text & "") Then
                    Consultar()
                    LimparCampos(Me)
                End If
            End If
