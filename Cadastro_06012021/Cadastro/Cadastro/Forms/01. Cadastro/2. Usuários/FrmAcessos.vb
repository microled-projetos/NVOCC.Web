...
Public Class FrmAcessos
    ...
    Private Sub ConsultarPermissoes(ByVal CodGrupo As String)
        ...
        SQL.Append("  FROM  ")
        SQL.Append("	" & Banco.BancoNVOCC & "TB_SYS_GRUPO_USER A ")
        SQL.Append("LEFT JOIN  ")
        SQL.Append("	" & Banco.BancoNVOCC & "TB_SYS_GRP_PERMISSOES B ON B.CodGrupo = A.CodGrupo ")
        SQL.Append("LEFT JOIN  ")
        SQL.Append("	" & Banco.BancoNVOCC & "TB_SYS_FUNCOES C ON B.CodFunc = C.CodFunc ")
        SQL.Append("LEFT JOIN  ")
        SQL.Append("	" & Banco.BancoNVOCC & "TB_SYS_TIPO_PERM D ON D.CODTIPOPERM = B.CODTIPOPERM ")
    ...
    Private Sub ConsultarGrupos()
        Me.cbGrupos.DataSource = Banco.List("SELECT CODGRUPO,DESCRICAO FROM " & Banco.BancoNVOCC & "TB_SYS_GRUPO_USER ORDER BY DESCRICAO")
    ...
    Private Sub btnSalvar_Click(sender As System.Object, e As System.EventArgs) Handles btnSalvar.Click
        ...
        For i As Integer = 0 To dgvConsulta.SelectedRows.Count - 1
            If Val(dgvConsulta.Rows(i).Cells(6).Value.ToString()) > 0 Then
                Banco.Execute("DELETE FROM " & Banco.BancoNVOCC & "TB_SYS_GRP_PERMISSOES WHERE CODFUNC = " & dgvConsulta.Rows(i).Cells(6).Value.ToString() & " AND CODGRUPO = " & cbGrupos.SelectedValue.ToString())
                For j As Integer = 1 To 5
                    If Val(dgvConsulta.Rows(i).Cells(j).Value.ToString()) > 0 Then
                        Banco.Execute("INSERT INTO " & Banco.BancoNVOCC & "TB_SYS_GRP_PERMISSOES (CODFUNC,CODGRUPO,CODTIPOPERM) VALUES (" & dgvConsulta.Rows(i).Cells(6).Value.ToString() & "," & cbGrupos.SelectedValue.ToString() & "," & j & ")")
                    End If
                Next
            End If
        Next
