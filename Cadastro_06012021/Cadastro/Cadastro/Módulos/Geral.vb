...
Module Geral
    ...
    Public Function Mensagens(ByVal Tela As Control, ByVal Tipo As Integer, Optional ByVal Custom As String = "") As String
        ...
        Select Case Tipo
            ...
            Case 8
                Return MessageBox.Show("Usuario não possui permissão. " & Custom, Tela.Text, MessageBoxButtons.OK, MessageBoxIcon.Exclamation)
        End Select
    ...
    Public Sub Gera_Rel_Servicos(ByVal _Lista_Preco As Long, Optional ByVal Modo As Integer = 0)
        ...
        Banco.Execute("DELETE FROM " & Banco.BancoNVOCC & "TEMP_REL_SERVICOS WHERE USUARIO=" & Banco.UsuarioSistema)

        PRSet(Tbhs, "SELECT A.OBS, A.Autonum, a.descr as Tabela,A.Flag_Liberada,  A.importador,a.dias_apos_gr,a.forma_pagamento, A.Data_Inicio, B.Razao as Nome_Parceiro,c.razao as Despachante,d.razao as Captador,e.descr as td, A.PROPOSTA FROM " & Banco.BancoNVOCC & "TB_LISTAS_PRECOS A, " & Banco.BancoNVOCC & "TB_CAD_PARCEIROS B," & Banco.BancoNVOCC & "TB_CAD_PARCEIROS c," & Banco.BancoNVOCC & "TB_CAD_PARCEIROS d," & Banco.BancoNVOCC & "TB_TIPOS_DOCUMENTOS e Where A.Autonum = " & Val(Lista_Preco) & "" & " And A.importador = B.Autonum (+) And A.despachante = c.Autonum (+) And A.captador = d.Autonum (+) and a.tipo_documento=e.code(+) order by A.descr", 0)
        Select Case Modo
            Case Is = 0
                'a.servico = b.autonum
                sql = "SELECT A.* FROM (  "
                sql = sql & " Select A.GRUPO_ATRACACAO, B.Descr as Servico, A.Tipo_Carga, A.Variante_Local, A.Base_Calculo, D.Descricao, A.Preco_Unitario, A.Preco_Minimo, A.Valor_Acrescimo, 'FIXOS' AS TIPOTABELA, VALOR_ACRESC_PESO,PESO_LIMITE,PRECO_MINIMO_DESOVA,TIPO_OPER,E.FANTASIA ,A.Autonum, b.autonum as numsv from " & Banco.BancoNVOCC & "TB_LISTA_PRECO_SERVICOS_FIXOS A, " & Banco.BancoNVOCC & "TB_SERVICOS_IPA  B, " & Banco.BancoNVOCC & "TB_CAD_MOEDAS D ," & Banco.BancoNVOCC & "TB_CAD_PARCEIROS E Where A.Lista  = '" & Lista_Preco & "' And A.Servico = B.Autonum(+) and A.Moeda = D.Autonum(+) AND A.PORTO_HUBPORT=E.AUTONUM(+) "
                sql = sql & " UNION ALL "
                sql = sql & " Select A.GRUPO_ATRACACAO, B.Descr as Servico, A.Tipo_Carga, A.Variante_Local, A.Base_Calculo, D.Descricao, A.Preco_Unitario, A.Preco_Minimo, A.Valor_Acrescimo, 'PERIODO' AS TIPOTABELA, VALOR_ACRESC_PESO,PESO_LIMITE,0,'','',A.Autonum, b.autonum as numsv from " & Banco.BancoNVOCC & "TB_LISTA_P_S_PERIODO A, " & Banco.BancoNVOCC & "TB_SERVICOS_IPA  B, " & Banco.BancoNVOCC & "TB_CAD_MOEDAS D Where A.Lista  = '" & Lista_Preco & "' And A.Servico = B.Autonum(+) and A.Moeda = D.Autonum(+) ) A WHERE TIPOTABELA = 'FIXOS' ORDER BY A.TIPOTABELA, A.SERVICO,A.Tipo_Carga"
                PRSet(tbhg1, sql, 0)
            Case Is = 1
                PRSet(tbhg1, "Select A.GRUPO_ATRACACAO, B.Descr as Servico, A.Tipo_Carga, A.Variante_Local, A.Base_Calculo, D.Descricao, A.Preco_Unitario, A.Preco_Minimo, A.Valor_Acrescimo,   VALOR_ACRESC_PESO,PESO_LIMITE,PRECO_MINIMO_DESOVA,TIPO_OPER,E.FANTASIA ,A.Autonum, b.autonum as numsv from " & Banco.BancoNVOCC & "TB_LISTA_PRECO_SERVICOS_FIXOS A, " & Banco.BancoNVOCC & "TB_SERVICOS_IPA  B, " & Banco.BancoNVOCC & "TB_CAD_MOEDAS D ," & Banco.BancoNVOCC & "TB_CAD_PARCEIROS E Where A.Lista  = " & Lista_Preco & "" & " And A.Servico = B.Autonum(+) and A.Moeda = D.Autonum(+) AND A.PORTO_HUBPORT=E.AUTONUM(+) and (a.autonum_vinculado is null or a.autonum_vinculado = 0) order by B.Descr,A.Tipo_Carga", 0)
            Case Is = 2
        ...
        If Not tbhg1.EOF Then
            ...
            If LPeriodo = True Then
                '----------------- Inicio Periodo ------------------------
                Select Case Modo
                    Case Is = 0 : PRSet(TBSERVPER, "Select  a.grupo_atracacao, B.Descr as Servico, A.N_Periodo, A.Qtde_Dias, A.Tipo_Carga, A.Variante_Local, A.Base_Calculo, D.Descricao, A.Preco_Unitario, A.Preco_Minimo, A.Valor_Acrescimo,decode(a.flag_prorata,0,'N',1,'S') as pr,  A.Autonum, a.servico as sipa from " & Banco.BancoNVOCC & "TB_LISTA_P_S_PERIODO A, " & Banco.BancoNVOCC & "TB_SERVICOS_IPA  B, " & Banco.BancoNVOCC & "TB_CAD_MOEDAS D Where A.Lista  = " & Lista_Preco & "" & " And A.Servico = B.Autonum(+) and A.Moeda = D.Autonum(+) order by b.descr,A.Tipo_Carga,a.n_periodo", 0)
                ...
                If Not TBSERVPER.EOF Then
                    ...
                    Do While Not TBSERVPER.EOF() = True
                        ...
                        If NNull(TBSERVPER.Fields("GRUPO_ATRACACAO").Value, 0) <> 0 Then
                            PRSet(TBLOCALATRAC, "SELECT DESCRICAO DESCR FROM " & Banco.BancoNVOCC & "TB_GRUPOS_ATRACACAO WHERE ID = " & NNull(TBSERVPER.Fields("GRUPO_ATRACACAO"), 0) & " ", 0)
                        ...
                        sql = sql & " from " & Banco.BancoNVOCC & "tb_servicos_ipa a, TB_lp_servicos_impostos b, tb_cad_impostos c"
            ...
            Do While Not tbhg1.EOF()
                ...
                If NNull(tbhg1.Fields("GRUPO_ATRACACAO").Value.ToString(), 0) <> 0 Then
                    PRSet(TBLOCALATRAC, "SELECT DESCRicao descr FROM " & Banco.BancoNVOCC & "tb_grupos_atracacao WHERE id = " & NNull(tbhg1.Fields("GRUPO_ATRACACAO").Value.ToString(), 0) & " ", 0)
            ...
            If Not LPeriodo = True Then
                '----------------- Inicio Periodo ------------------------
                PRSet(TBSERVPER, "Select  a.grupo_atracacao, B.Descr as Servico, A.N_Periodo, A.Qtde_Dias, A.Tipo_Carga, A.Variante_Local, A.Base_Calculo, D.Descricao, A.Preco_Unitario, A.Preco_Minimo, A.Valor_Acrescimo,  decode(a.flag_prorata,0,'N',1,'S') as pr, A.Autonum from " & Banco.BancoNVOCC & "TB_LISTA_P_S_PERIODO A, " & Banco.BancoNVOCC & "TB_SERVICOS_IPA  B, " & Banco.BancoNVOCC & "TB_CAD_MOEDAS D Where A.Lista  = " & Lista_Preco & "" & " And A.Servico = B.Autonum(+) and A.Moeda = D.Autonum(+) order by b.descr", 0)
                If Not TBSERVPER.EOF Then
                    Lista_Preco = Val(Tbhs.Fields("Autonum").Value.ToString())
                    Cabecalho()
                    ...
                    Do While Not TBSERVPER.EOF()
                        ...
                        If NNull(TBSERVPER.Fields("grupo_ATRACACAO").Value.ToString(), 0) <> 0 Then
                            PRSet(TBLOCALATRAC, "SELECT DESCRicao descr FROM " & Banco.BancoNVOCC & "tb_grupos_atracacao WHERE id = " & NNull(TBSERVPER.Fields("grupo_ATRACACAO").Value.ToString(), 0) & " ", 0)
    ...
    Private Sub Cabecalho()
        ...
        If LinhaRelServicos Mod 35 = 0 Or LinhaRelServicos = 0 Then
            ...
            If LinhaRelServicos < 31 Then
                ...
                PRSet(TBV, "Select a.razao from " & Banco.BancoNVOCC & "tb_cad_parceiros a, " & Banco.BancoNVOCC & "tb_listas_precos_vendedores b where b.vendedor = a.autonum and b.lista = " & Tbhs.Fields("Autonum").Value.ToString())
                ...
                TBV.Close()
                Call Acrescenta_Linha_Relatorio_Servicos(Lista_Preco, LinhaRelServicos, TEXTO, Banco.UsuarioSistema)
                sql = "select lote from " & Banco.BancoNVOCC & "tb_lp_lotes where autonum_lista=" & Tbhs.Fields("Autonum").Value.ToString() & " order by lote"
                PRSet(TBV, sql)
                TEXTO = ""
                Do While Not TBV.EOF
    ...
    Public Sub Acrescenta_Linha_Relatorio_Servicos(ByVal Campo1 As Long, ByVal Campo2 As Integer, ByVal Campo3 As String, ByVal Campo4 As Long)
        ...
        sql = "INSERT INTO " & Banco.BancoNVOCC & "TEMP_REL_SERVICOS (Lista_Preco, linha, texto,usuario) values "
