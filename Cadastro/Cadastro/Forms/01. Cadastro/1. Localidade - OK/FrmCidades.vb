...
Public Class FrmCidades
    ...
    Private Sub ConsultarEstados()
        cbUF.DataSource = Banco.List("SELECT ID_ESTADO, NM_ESTADO DESCR FROM TB_ESTADO ORDER BY DESCR")
        cbUF.SelectedIndex = -1
    End Sub

    Private Sub Consultar()
        dgvConsulta.DataSource = Banco.List("SELECT a.ID_CIDADE AS ID ,A.COD_IBGE as IBGE, A.NM_CIDADE as CIDADE, B.NM_ESTADO as ESTADO, C.NM_PAIS as PAIS, A.VL_ISS as ISS, A.ID_ESTADO,A.ID_PAIS FROM TB_CIDADE A LEFT JOIN TB_ESTADO B on A.ID_ESTADO = B.ID_ESTADO LEFT JOIN TB_PAIS C on A.ID_PAIS = C.ID_PAIS WHERE NOT A.COD_IBGE IS NULL ORDER BY ESTADO,CIDADE")
...
FROM [TB_GRUPO_PERMISSAO] where ID_Menu = 12 AND ID_TIPO_USUARIO IN( SELECT distinct ID_TIPO_USUARIO from TB_VINCULO_USUARIO where ID_USUARIO = " & Conversions.ToString(UsuarioSistema) & ")  ")
    ...
    Private Sub btnExcluir_Click(sender As Object, e As EventArgs) Handles btnExcluir.Click
        If Not String.IsNullOrEmpty(txtCodigo.Text) AndAlso MessageBox.Show("Deseja realmente excluir o registro selecionado?", Text, MessageBoxButtons.YesNo, MessageBoxIcon.Question) = DialogResult.Yes AndAlso Banco.Execute("DELETE FROM TB_CIDADE WHERE COD_IBGE = " + txtIBGE.Text & " AND ID_CIDADE = " + txtCodigo.Text) Then
            Consultar()
            Dim Tela As Control = Me
            Geral.LimparCampos(Tela)
        End If
