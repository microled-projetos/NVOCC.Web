...
Public Class FrmUsuarios
    ...
    Private Sub ConsultarPatios()
        Me.cbPatio.DataSource = Banco.List("SELECT AUTONUM,DESCR_RESUMIDO AS DESCR FROM " & Banco.BancoOPERADOR & "TB_PATIOS")
    ...
    Private Sub ConsultarGrupos()
        ...
        ds = Banco.List("SELECT CODGRUPO, CONVERT(VARCHAR,CODGRUPO) + ' - ' + DESCRICAO AS DESCRICAO FROM " & Banco.BancoSGIPA & "TB_SYS_GRUPO_USER ORDER BY DESCRICAO")
    ...
    Private Sub ConsultarServicosTerminal()
        ...
        Dim ds As New DataTable
        ds = Banco.List("SELECT CODE, CONVERT(VARCHAR,CODE) + ' - ' + DESCR AS DESCR FROM " & Banco.BancoSGIPA & "TB_MOTIVO_POSICAO ORDER BY DESCR")
    ...
    Private Sub ConsultarEmpresas()
        Me.cbEmpresa.DataSource = Banco.List("SELECT AUTONUM,NOME_FANTASIA AS DESCRICAO FROM " & Banco.BancoSGIPA & "TB_EMPRESAS")
        Me.cbEmpresa.SelectedIndex = -1
    End Sub

    Private Sub ConsultarVendedores()
        Me.cbVendedor.DataSource = Banco.List("SELECT AUTONUM,FANTASIA FROM " & Banco.BancoSGIPA & "TB_CAD_PARCEIROS WHERE FLAG_VENDEDOR = 1")
    ...
    Private Sub Consultar(ByVal Chave As String)
        ...
        If Me.txtBuscarUsuario.Text <> String.Empty Then
            Me.dgvConsulta.DataSource = Banco.List("SELECT AUTONUM,USUARIO,SENHA,ISNULL(PATIO,0) PATIO,NOME,EMAIL,ISNULL(ID_VENDEDOR,0) ID_VENDEDOR,ISNULL(COD_EMPRESA,0) COD_EMPRESA,CPF, FLAG_ATIVO FROM " & Banco.BancoSGIPA & "TB_CAD_USUARIOS WHERE USUARIO LIKE '%" & Chave & "%' OR NOME LIKE '%" & Chave & "%'")
        Else
            Me.dgvConsulta.DataSource = Banco.List("SELECT AUTONUM,USUARIO,SENHA,ISNULL(PATIO,0) PATIO,NOME,EMAIL,ISNULL(ID_VENDEDOR,0) ID_VENDEDOR,ISNULL(COD_EMPRESA,0) COD_EMPRESA,CPF, FLAG_ATIVO FROM " & Banco.BancoSGIPA & "TB_CAD_USUARIOS")
        End If
    ...
    Private Sub MostraDados()
        ...
        If Me.dgvConsulta.Rows.Count > 0 Then
            ...
            Ds = Banco.List("SELECT " & SQL.ToString() & " FROM " & Banco.BancoSGIPA & "TB_CAD_USUARIOS WHERE AUTONUM = " & Me.txtCodigo.Text)
            ...
            Ds = Banco.List("SELECT CODGRUPO FROM " & Banco.BancoSGIPA & "TB_SYS_USER_GRUPOS WHERE AUTONUMUSER = " & txtCodigo.Text)
            ...
            Ds = Banco.List("SELECT A.AUTONUM_POSICIONAMENTO FROM " & Banco.BancoSGIPA & "TB_AMR_USU_POSICIONAMENTO A LEFT JOIN " & Banco.BancoSGIPA & "TB_MOTIVO_POSICAO B ON A.AUTONUM_POSICIONAMENTO = B.CODE WHERE A.AUTONUM_USUARIO = " & txtCodigo.Text)
    ...
    Private Sub btnSalvar_Click(sender As System.Object, e As System.EventArgs) Handles btnSalvar.Click
            ...
            Try
                ...
                If Banco.Execute(sSql) Then
                    ...
                    For Each Item As String In lstGrupos.CheckedItems
                        Banco.Execute("INSERT INTO " & Banco.BancoSGIPA & "TB_SYS_USER_GRUPOS (AUTONUMUSER,CODGRUPO) VALUES (" & Codigo & "," & Convert.ToInt32(Item.Split("-")(0).ToString().Trim) & ")")
                    Next

                    For Each Item As String In lstServicos.CheckedItems
                        Banco.Execute("INSERT INTO " & Banco.BancoSGIPA & "TB_AMR_USU_POSICIONAMENTO (AUTONUM,AUTONUM_USUARIO,AUTONUM_POSICIONAMENTO) VALUES (" & Banco.BancoSGIPA & "SEQ_AMR_USU_SERVICO.NEXTVAL," & Codigo & ", " & Convert.ToInt32(Item.Split("-")(0).ToString().Trim) & ")")
                    Next
        ...
        Else
            Try
                ...
                sSql = "UPDATE " & Banco.BancoSGIPA & "TB_CAD_USUARIOS SET USUARIO = '" & txtUsuario.Text & "',SENHA = '" & txtSenha.Text & "',PATIO = " & IIf(cbPatio.SelectedValue IsNot Nothing, cbPatio.SelectedValue, "NULL") & ",NOME = '" & txtNome.Text & "',EMAIL = '" & txtEmail.Text & "',ID_VENDEDOR = " & IIf(cbVendedor.SelectedValue IsNot Nothing, cbVendedor.SelectedValue, "NULL") & ",COD_EMPRESA = " & IIf(cbEmpresa.SelectedValue IsNot Nothing, cbEmpresa.SelectedValue, "0") & ",CPF = '" & txtCPF.Text & "', " & Campos.ToString() & " WHERE AUTONUM = " & txtCodigo.Text

                If Banco.Execute(sSql) Then

                    Banco.Execute("DELETE FROM " & Banco.BancoSGIPA & "TB_SYS_USER_GRUPOS WHERE AUTONUMUSER = " & txtCodigo.Text)
                    Banco.Execute("DELETE FROM " & Banco.BancoSGIPA & "TB_AMR_USU_POSICIONAMENTO WHERE AUTONUM_USUARIO = " & txtCodigo.Text)

                    For Each Item As String In lstGrupos.CheckedItems
                        Banco.Execute("INSERT INTO " & Banco.BancoSGIPA & "TB_SYS_USER_GRUPOS (AUTONUMUSER,CODGRUPO) VALUES (" & txtCodigo.Text & "," & Convert.ToInt32(Item.Split("-")(0).ToString().Trim) & ")")
                    Next

                    For Each Item As String In lstServicos.CheckedItems
                        Banco.Execute("INSERT INTO " & Banco.BancoSGIPA & "TB_AMR_USU_POSICIONAMENTO (AUTONUM,AUTONUM_USUARIO,AUTONUM_POSICIONAMENTO) VALUES (" & Banco.BancoSGIPA & "SEQ_AMR_USU_SERVICO.NEXTVAL," & txtCodigo.Text & ", " & Convert.ToInt32(Item.Split("-")(0).ToString().Trim) & ")")
                    Next
    ...
    Private Sub btnExcluir_Click(sender As System.Object, e As System.EventArgs) Handles btnExcluir.Click
        ...
        If Not String.IsnullOrEmpty(txtCodigo.Text) Then
            If MessageBox.Show("Deseja realmente excluir o registro selecionado?", Me.Text, MessageBoxButtons.YesNo, MessageBoxIcon.Question) = Windows.Forms.DialogResult.Yes Then
                If Banco.Execute("DELETE FROM " & Banco.BancoSGIPA & "TB_CAD_USUARIOS WHERE AUTONUM = " & txtCodigo.Text & "") Then
                    Consultar(String.Empty)
                    LimparCampos(Me)
                End If
            End If
    ...
    Private Sub btImprimir_Click(sender As System.Object, e As System.EventArgs)
        ...
        Me.cbGrupoRel.DataSource = Banco.List("SELECT DISTINCT TB_SYS_GRUPO_USER.DESCRICAO, TB_SYS_GRUPO_USER.CODGRUPO FROM " & Banco.BancoSGIPA & "TB_SYS_GRUPO_USER JOIN " & Banco.BancoSGIPA & "TB_SYS_USER_GRUPOS ON TB_SYS_GRUPO_USER.CodGrupo = TB_SYS_USER_GRUPOS.CodGrupo JOIN " & Banco.BancoSGIPA & "TB_CAD_USUARIOS ON TB_SYS_USER_GRUPOS.AUTONUMUSER = TB_CAD_USUARIOS.AUTONUM WHERE TB_CAD_USUARIOS.FLAG_ATIVO=1 ORDER BY DESCRICAO")
        Me.cbUsuarioRel.DataSource = Banco.List("SELECT USUARIO,AUTONUM FROM " & Banco.BancoSGIPA & "TB_CAD_USUARIOS WHERE FLAG_ATIVO = 1")
        Me.cbModuloRel.DataSource = Banco.List("SELECT DISTINCT SISTEMA FROM " & Banco.BancoSGIPA & "TB_SYS_GRUPO_USER JOIN " & Banco.BancoSGIPA & "TB_SYS_USER_GRUPOS ON TB_SYS_GRUPO_USER.CodGrupo = TB_SYS_USER_GRUPOS.CodGrupo JOIN " & Banco.BancoSGIPA & "TB_SYS_GRP_PERMISSOES TB_SYS_GRP_PERMISSOES ON TB_SYS_USER_GRUPOS.CodGrupo = TB_SYS_GRP_PERMISSOES.CodGrupo JOIN " & Banco.BancoSGIPA & "TB_SYS_FUNCOES ON TB_SYS_GRP_PERMISSOES.CodFunc = TB_SYS_FUNCOES.CodFunc JOIN " & Banco.BancoSGIPA & "TB_CAD_USUARIOS ON TB_SYS_USER_GRUPOS.AUTONUMUSER = TB_CAD_USUARIOS.AUTONUM WHERE TB_CAD_USUARIOS.FLAG_ATIVO=1 AND TB_SYS_FUNCOES.sistema <> 'MICROLED' ORDER BY SISTEMA")
    ...
    Private Sub btnImprimirRel_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnImprimirRel.Click
        ...
        Select Case TipoRel
            Case "1" 'Relatório de Grupos.
                ...
                Sql = Sql & " FROM   " & Banco.BancoSGIPA & "TB_SYS_GRUPO_USER TB_SYS_GRUPO_USER JOIN  "
                Sql = Sql & " " & Banco.BancoSGIPA & "TB_SYS_USER_GRUPOS TB_SYS_USER_GRUPOS ON TB_SYS_GRUPO_USER.CodGrupo = TB_SYS_USER_GRUPOS.CodGrupo JOIN "
                Sql = Sql & " " & Banco.BancoSGIPA & "TB_SYS_GRP_PERMISSOES TB_SYS_GRP_PERMISSOES ON TB_SYS_USER_GRUPOS.CodGrupo = TB_SYS_GRP_PERMISSOES.CodGrupo JOIN "
                Sql = Sql & " " & Banco.BancoSGIPA & "TB_SYS_FUNCOES TB_SYS_FUNCOES ON TB_SYS_GRP_PERMISSOES.CodFunc = TB_SYS_FUNCOES.CodFunc JOIN "
                Sql = Sql & " " & Banco.BancoSGIPA & "TB_SYS_TIPO_PERM TB_SYS_TIPO_PERM ON TB_SYS_GRP_PERMISSOES.CODTIPOPERM = TB_SYS_TIPO_PERM.CODTIPOPERM JOIN "
                Sql = Sql & " " & Banco.BancoSGIPA & "TB_CAD_USUARIOS TB_CAD_USUARIOS ON TB_SYS_USER_GRUPOS.AUTONUMUSER = TB_CAD_USUARIOS.AUTONUM "
            ...
            Case "3" 'Relatório de Grupos por Módulo.
                ...
                sql = sql & " FROM   " & Banco.BancoSGIPA & "TB_SYS_GRUPO_USER TB_SYS_GRUPO_USER  "
                sql = sql & "        join  " & Banco.BancoSGIPA & "TB_SYS_USER_GRUPOS TB_SYS_USER_GRUPOS on (TB_SYS_GRUPO_USER.CODGRUPO=TB_SYS_USER_GRUPOS.CODGRUPO) "
                sql = sql & "        join  " & Banco.BancoSGIPA & "TB_SYS_GRP_PERMISSOES TB_SYS_GRP_PERMISSOES on  (TB_SYS_USER_GRUPOS.CODGRUPO=TB_SYS_GRP_PERMISSOES.CODGRUPO) "
                sql = sql & "        join  " & Banco.BancoSGIPA & "TB_SYS_FUNCOES TB_SYS_FUNCOES on (TB_SYS_GRP_PERMISSOES.CODFUNC=TB_SYS_FUNCOES.CODFUNC) "
