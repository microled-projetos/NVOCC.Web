...
Module Inicio
    ...
    Public Sub DownloadNF()
        ...
        Try
            ...
            Dim ds As DataSet = Con.ExecutarQuery(" SELECT A.ID_FATURAMENTO, A.NR_NOTA_FISCAL, A.COD_VER_NFSE, B.CNPJ FROM TB_FATURAMENTO A CROSS JOIN TB_EMPRESAS B WHERE A.NR_RPS IS NOT NULL AND A.ID_STATUS_DOWNLOAD_NFE = 0 AND A.NR_NOTA_FISCAL IS NOT NULL AND A.COD_VER_NFSE IS NOT NULL AND A.DT_CANCELAMENTO IS NULL ORDER BY A.ID_FATURAMENTO DESC ")
            If ds.Tables(0).Rows.Count > 0 Then
                For Each linhads As DataRow In ds.Tables(0).Rows
    ...
    Sub EnviaEmail()
        ...
        Try
            ...
            If rsParam.Tables(0).Rows.Count > 0 Then
                ...
                rsDados = Con.ExecutarQuery(" SELECT TOP 3 A.ID_FATURAMENTO,A.NR_NOTA_FISCAL,A.VL_NOTA,A.COD_VER_NFSE,A.DT_NOTA_FISCAL,A.NM_CLIENTE,A.CNPJ,B.EMAIL_NF_ELETRONICA 
FROM TB_FATURAMENTO A
INNER JOIN TB_PARCEIRO B ON A.ID_PARCEIRO_CLIENTE = B.ID_PARCEIRO
WHERE A.ID_STATUS_DOWNLOAD_NFE <> 0 AND A.FL_STATUS_EMAIL_NFE = 0 AND B.EMAIL_NF_ELETRONICA IS NOT NULL AND A.DT_CANCELAMENTO IS NULL")
                ...
                If rsParam.Tables(0).Rows.Count > 0 Then
                    ...
                    For Each linhads As DataRow In rsDados.Tables(0).Rows
                        ...
                        'DESTINATARIO
                        enderecos = "juliane@microled.com.br"
