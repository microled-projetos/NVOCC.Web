    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        Else
            If Request.QueryString("id") <> "" Then
                ...
                ds = Con.ExecutarQuery("SELECT (SELECT NM_RAZAO FROM TB_PARCEIRO WHERE ID_PARCEIRO = D.ID_PARCEIRO_IMPORTADOR)NM_CLIENTE,(SELECT ID_CIDADE FROM TB_PARCEIRO WHERE ID_PARCEIRO = D.ID_PARCEIRO_IMPORTADOR)ID_CIDADE,
SUM(C.VL_DETENTION_VENDA)VL_DETENTION_VENDA, SUM(C.VL_DETENTION_VENDA_BR)VL_DETENTION_VENDA_BR ,
SUM(C.VL_DETENTION_LIQUIDO_VENDA)VL_DETENTION_LIQUIDO_VENDA,
NR_BL,NR_PROCESSO, (SELECT NR_BL FROM TB_BL M WHERE M.ID_BL = D.ID_BL_MASTER)NR_BL_MASTER,(SELECT top 1 NR_REFERENCIA_CLIENTE FROM VW_REFERENCIA_CLIENTE WHERE ID_BL = D.ID_BL_MASTER)NR_REFERENCIA_CLIENTE,CONVERT(VARCHAR,A.DT_LANCAMENTO,103)DT_LANCAMENTO,A.NR_RECIBO
FROM TB_DETENTION_FATURA A
INNER JOIN TB_DETENTION_FATURA_ITENS B ON A.ID_DETENTION_FATURA = B.ID_DETENTION_FATURA
INNER JOIN TB_CNTR_DETENTION C ON B.ID_CNTR_DETENTION = C.ID_CNTR_DETENTION
LEFT  JOIN TB_BL D ON A.ID_BL = D.ID_BL
WHERE A.ID_DETENTION_FATURA = " & ID & "
 GROUP BY ID_PARCEIRO_IMPORTADOR,NR_BL,NR_PROCESSO,ID_BL_MASTER,A.DT_LANCAMENTO,A.NR_RECIBO")
                If ds.Tables(0).Rows.Count > 0 Then
                    ...
                    Else
                        Dim numero As String = NumeracaoDoc.Numerar(5)
                        lblNumeroRecibo.Text = numero
                        Con.ExecutarQuery("UPDATE [dbo].[TB_DETENTION_FATURA] SET NR_RECIBO = '" & numero & "' WHERE ID_DETENTION_FATURA =" & ID)
                        Con.ExecutarQuery("UPDATE [dbo].[TB_NUMERACAO] SET NR_RECIBO_DETENTION = '" & numero & "' WHERE ID_NUMERACAO = 5")
                        Con.Fechar()
                    End If
                    ...
                    tabela &= "<tr><td style='padding-right:10px'>DETENTION</td>"
                    tabela &= "<td style='padding-left:10px;padding-right:10px'>" & ds.Tables(0).Rows(0).Item("VL_DETENTION_LIQUIDO_VENDA") & "</td>"
                    tabela &= "<td style='padding-left:10px;padding-right:10px'></td></tr>"
                    valores = valores + ds.Tables(0).Rows(0).Item("VL_DETENTION_LIQUIDO_VENDA")
                    If CIDADE = "78" Then
