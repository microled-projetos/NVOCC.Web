Public Class CalculaBL
    Public Function Calcular(ID_BL As Integer) As String
        ...
        Dim msg6 As String = "BL Master:Não há valor de moeda de câmbio cadastrado com a data atual."

        '   Dim msg7 As String = "BL Master:Base de Calculo não informada."

        Dim msg8 As String = "Não há taxas vinculadas a BL Master."
        ...
        Con.Conectar()
        Dim ds As DataSet = Con.ExecutarQuery("SELECT B.ID_BL_TAXA, isnull(B.VL_TAXA,0)VL_TAXA,B.ID_MOEDA,isnull(B.VL_CAMBIO,0)VL_CAMBIO,B.ID_BASE_CALCULO_TAXA,isnull(A.VL_M3,0)VL_M3,isnull(A.VL_PESO_BRUTO,0)VL_PESO_BRUTO,
...
C.ID_CONTA_PAGAR_RECEBER_ITENS, D.DT_CANCELAMENTO 
FROM TB_BL  A
LEFT JOIN TB_BL_TAXA B ON A.ID_BL = B.ID_BL 
LEFT JOIN TB_CONTA_PAGAR_RECEBER_ITENS C ON C.ID_BL_TAXA = B.ID_BL_TAXA  
LEFT JOIN TB_CONTA_PAGAR_RECEBER D ON C.ID_CONTA_PAGAR_RECEBER = D.ID_CONTA_PAGAR_RECEBER AND D.DT_CANCELAMENTO IS NOT NULL 
 WHERE  ID_BASE_CALCULO_TAXA IS NOT NULL AND B.ID_BL_TAXA IS NOT NULL AND ID_BASE_CALCULO_TAXA <> 1 AND A.ID_BL  =" & ID_BL)
...
WHERE A.ID_BL = " & ID_BL & " AND ID_SERVICO IN (1,4) AND GRAU = 'C' ")
        ...
        Dim dsMaster As DataSet = Con.ExecutarQuery("SELECT B.ID_BL_TAXA, isnull(B.VL_TAXA,0)VL_TAXA,B.ID_MOEDA,isnull(B.VL_CAMBIO,0)VL_CAMBIO,B.ID_BASE_CALCULO_TAXA,isnull(A.VL_M3,0)VL_M3,isnull(A.VL_PESO_BRUTO,0)VL_PESO_BRUTO,
CASE WHEN B.ID_MOEDA = 124 THEN CONVERT(VARCHAR, GETDATE(), 103) ELSE
(SELECT CONVERT(VARCHAR,MAX(DT_CAMBIO),103) FROM TB_MOEDA_FRETE WHERE ID_MOEDA = B.ID_MOEDA) END DT_CAMBIO,
C.ID_CONTA_PAGAR_RECEBER_ITENS, D.DT_CANCELAMENTO 
FROM TB_BL  A
...
LEFT JOIN TB_CONTA_PAGAR_RECEBER_ITENS C ON C.ID_BL_TAXA = B.ID_BL_TAXA  
LEFT JOIN TB_CONTA_PAGAR_RECEBER D ON C.ID_CONTA_PAGAR_RECEBER = D.ID_CONTA_PAGAR_RECEBER AND D.DT_CANCELAMENTO IS NOT NULL 
 WHERE ID_BASE_CALCULO_TAXA IS NOT NULL AND B.ID_BL_TAXA IS NOT NULL AND ID_BASE_CALCULO_TAXA <> 1 AND A.ID_BL = (SELECT ID_BL_MASTER FROM TB_BL WHERE ID_BL = " & ID_BL & ")")
        ...
        WHERE A.ID_BL =  " & ID_BL)
                ...
                ElseIf linha.Item("ID_BASE_CALCULO_TAXA") = 4 Then
                    'TOTAL DO HOUSE
                    Dim ds1 As DataSet = Con.ExecutarQuery("SELECT COUNT(DISTINCT ID_BL)QTD FROM TB_BL where GRAU = 'C' AND ID_BL_MASTER = " & ID_BL)
                    x = ds1.Tables(0).Rows(0).Item("QTD")
                    y = linha.Item("VL_TAXA")
                    z = y * x
                    Taxa = z.ToString
...
WHERE A.ID_BL_MASTER = " & ID_BL & " AND ID_SERVICO IN (1,4) AND GRAU = 'C' ")
        ...
        WHERE A.ID_BL_MASTER =" & ID_BL)
                ...
                ElseIf linha.Item("ID_BASE_CALCULO_TAXA") = 35 Then
                    ...
                    'Para cada conteiner de 20' corresponde 1 teu
                    Dim ds1 As DataSet = Con.ExecutarQuery("Select count(ID_CNTR_BL)QTD FROM TB_CNTR_BL A
        WHERE A.ID_BL_MASTER = " & ID_BL & " AND ID_TIPO_CNTR in (5,6,2,9,10,12,16,18)")
                    y = ds1.Tables(0).Rows(0).Item("QTD")
                    ...
                    ds1 = Con.ExecutarQuery("Select count(ID_CNTR_BL)QTD FROM TB_CNTR_BL A
        WHERE A.ID_BL_MASTER = " & ID_BL & " AND ID_TIPO_CNTR In (19,17,13,14,15,11,3,4,7,8,1)")
                    x = ds1.Tables(0).Rows(0).Item("QTD")
                    x = x * 2
