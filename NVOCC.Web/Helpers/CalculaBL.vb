Public Class CalculaBL
    Public Function Calcular(ID_BL As Integer) As String
        ...
        Dim msg5 As String = "Não foi possível calcular BL Master: a taxa já foi enviada para contas a pagar/receber!."
...
WHERE A.ID_BL = " & ID_BL & " AND ID_SERVICO IN (1,4) AND GRAU = 'C' ")
        ...
        Dim dsMaster As DataSet = Con.ExecutarQuery("select A.ID_BL,B.ID_BL_TAXA, isnull(B.VL_TAXA,0)VL_TAXA,B.ID_MOEDA,isnull(B.VL_CAMBIO,0)VL_CAMBIO,B.ID_BASE_CALCULO_TAXA,isnull(A.VL_M3,0)VL_M3,isnull(A.VL_PESO_BRUTO,0)VL_PESO_BRUTO,(SELECT MAX(DT_CAMBIO) FROM TB_MOEDA_FRETE WHERE ID_MOEDA = B.ID_MOEDA)DT_CAMBIO,C.ID_CONTA_PAGAR_RECEBER_ITENS, D.DT_CANCELAMENTO 
from TB_BL  A
LEFT JOIN TB_BL_TAXA B ON A.ID_BL = B.ID_BL 
LEFT JOIN TB_CONTA_PAGAR_RECEBER_ITENS C ON C.ID_BL_TAXA = B.ID_BL_TAXA  
LEFT JOIN TB_CONTA_PAGAR_RECEBER D ON C.ID_CONTA_PAGAR_RECEBER = D.ID_CONTA_PAGAR_RECEBER 
