...
(SELECT (ISNULL(SUM(VL_CARGA),0)) FROM TB_COTACAO_MERCADORIA B WHERE A.ID_COTACAO = B.ID_COTACAO )VL_CARGA ,isnull(A.VL_PESO_TAXADO,0)VL_PESO_TAXADO,isnull(A.FL_TC4,0)FL_TC4,isnull(A.FL_TC6,0)FL_TC6 FROM TB_COTACAO A WHERE A.ID_COTACAO = " & ID_COTACAO)
    ...
    Sub UpdateFrete(ID_COTACAO As String, NR_PROCESSO As String)
        ...
        Else
            ...
            Dim dsCotacao As DataSet = Con.ExecutarQuery("SELECT ID_TIPO_ESTUFAGEM, ID_SERVICO , ID_PORTO_ORIGEM,ID_PORTO_DESTINO,ID_PORTO_ESCALA1,FINAL_DESTINATION,ID_PORTO_ESCALA2,ID_PORTO_ESCALA3,ID_MOEDA_FRETE ,ID_TIPO_CARGA,ID_TRANSPORTADOR,ID_COTACAO,VL_DIVISAO_FRETE,ID_TIPO_DIVISAO_FRETE,ID_TIPO_PAGAMENTO,NR_CONTRATO_ARMADOR, ISNULL(ID_TIPO_AERONAVE,0)ID_TIPO_AERONAVE FROM TB_COTACAO WHERE ID_COTACAO = " & ID_COTACAO)
            ...
            If dsCotacao.Tables(0).Rows(0).Item("ID_TRANSPORTADOR").ToString <> dsProcesso.Tables(0).Rows(0).Item("ID_PARCEIRO_TRANSPORTADOR").ToString Then
                Con.ExecutarQuery("UPDATE TB_BL SET ID_PARCEIRO_TRANSPORTADOR = " & dsCotacao.Tables(0).Rows(0).Item("ID_TRANSPORTADOR").ToString & " WHERE ((ID_BL_MASTER IS NULL) OR (ID_TIPO_ESTUFAGEM = 1)) AND ID_BL = " & ID_BL)
            End If
...
(SELECT (ISNULL(SUM(VL_CARGA),0)) FROM TB_COTACAO_MERCADORIA B WHERE A.ID_COTACAO = B.ID_COTACAO )VL_CARGA,  
 ...
 CASE WHEN ISNULL(ID_DESTINATARIO_COBRANCA,0) = 1  THEN ID_CLIENTE 
 WHEN ISNULL(ID_DESTINATARIO_COBRANCA,0) = 2  THEN ID_AGENTE_INTERNACIONAL
 WHEN ISNULL(ID_DESTINATARIO_COBRANCA,0) = 3  THEN 0 
 WHEN ISNULL(ID_DESTINATARIO_COBRANCA,0) = 4  THEN ID_PARCEIRO_IMPORTADOR
 WHEN ISNULL(ID_DESTINATARIO_COBRANCA,0) = 7  THEN ID_PARCEIRO_RODOVIARIO
 WHEN ISNULL(ID_DESTINATARIO_COBRANCA,0) = 8  THEN ID_PARCEIRO_EXPORTADOR
 ELSE ID_CLIENTE  END ID_PARCEIRO_EMPRESA, 
 
 ISNULL(ID_DESTINATARIO_COBRANCA,0)ID_DESTINATARIO_COBRANCA, 
