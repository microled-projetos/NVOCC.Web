...
WHERE A.ID_COTACAO =  " & ID_COTACAO)
                    ...
                    ElseIf linha.Item("ID_BASE_CALCULO_TAXA") = 4 Then
                        ...
                        Dim ds1 As DataSet = Con.ExecutarQuery("SELECT 
  ISNULL((SELECT sum(VL_TAXA_VENDA_CALCULADO) from TB_COTACAO_TAXA B  where B.ID_COTACAO = A.ID_COTACAO AND B.FL_DECLARADO = 1 AND 
A.ID_MOEDA_FRETE = B.ID_MOEDA_VENDA),0) +  VL_TOTAL_FRETE_VENDA_CALCULADO  as TOTAL_VENDA ,
  ISNULL((SELECT sum(VL_TAXA_COMPRA_CALCULADO) from TB_COTACAO_TAXA B  where B.ID_COTACAO = A.ID_COTACAO AND B.FL_DECLARADO = 1 AND 
A.ID_MOEDA_FRETE = B.ID_MOEDA_COMPRA),0) +  VL_TOTAL_FRETE_COMPRA  as TOTAL_COMPRA 
 FROM TB_COTACAO A
 WHERE  
