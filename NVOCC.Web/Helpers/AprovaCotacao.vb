...
FROM TB_COTACAO_TAXA WHERE VL_TAXA_VENDA IS NOT NULL AND VL_TAXA_VENDA <> 0 AND  ID_COTACAO = " & ID_COTACAO)
        ...
        Dim ID_BASE_CALCULO As Integer = 0




        If ID_SERVICO = 2 Or ID_SERVICO = 5 Then ''AEREO
            ID_BASE_CALCULO = 42
 ...
 ISNULL(ID_DESTINATARIO_COBRANCA,0),
...
ELSE 0
 ...
 SELECT (SELECT ID_ITEM_FRETE_MASTER FROM TB_PARAMETROS)," & ID_BASE_CALCULO & ",ID_MOEDA_FRETE,VL_TOTAL_FRETE_COMPRA,VL_TOTAL_FRETE_COMPRA,VL_TOTAL_FRETE_COMPRA_MIN," & ID_BL & ",'P', " & FL_PROFIT_FRETE & ",
  ID_TRANSPORTADOR AS ID_PARCEIRO_EMPRESA,ISNULL(ID_DESTINATARIO_COBRANCA,0), 1,'COTA'  
 FROM TB_COTACAO WHERE ID_COTACAO = " & ID_COTACAO)
...
WHERE A.ID_COTACAO_MERCADORIA =" & linha.Item("ID_COTACAO_MERCADORIA"))
        ...
        ds = Con.ExecutarQuery("SELECT ISNULL(ID_TIPO_PAGAMENTO,0)ID_TIPO_PAGAMENTO,ISNULL(FL_FREE_HAND,0)FL_FREE_HAND FROM TB_BL WHERE ID_BL = " & ID_BL)
        Dim AtualizaStatus As New AtualizaStatusFreteAgente
        Dim STATUS = AtualizaStatus.AtualizaStatusFreteAgenteHouse(ID_BL, ds.Tables(0).Rows(0).Item("ID_TIPO_PAGAMENTO"), ds.Tables(0).Rows(0).Item("FL_FREE_HAND"))
    ...
    Sub CancelaCotacaoRepetidas(ID_COTACAO As Integer, ID_USUARIO As String, NR_COTACAO As String)
        ...
        Con.ExecutarQuery(" UPDATE TB_COTACAO SET ID_STATUS_COTACAO = 17, DT_STATUS_COTACAO= GETDATE(), ID_USUARIO_STATUS = " & ID_USUARIO & ", OB_MOTIVO_CANCELAMENTO = 'COTAÇÃO REPETIDA: Cancelada devido a aprovação da cotação " & NR_COTACAO & "' WHERE ID_COTACAO <> " & ID_COTACAO & " AND NR_PROCESSO_GERADO IS NULL AND REF_REPETIDAS IN (SELECT B.REF_REPETIDAS FROM TB_COTACAO B WHERE B.ID_COTACAO = " & ID_COTACAO & " )")
