Public Class CadastrarFreteTransportador
    ...
    Sub CarregaCampos()
        ...
        ds = Con.ExecutarQuery("SELECT A.ID_FRETE_TRANSPORTADOR,A.ID_TRANSPORTADOR,A.ID_AGENTE,A.ID_PORTO_ORIGEM,A.ID_PORTO_DESTINO,A.ID_MOEDA_FRETE,A.ID_TIPO_CARGA,A.ID_VIA_ROTA,A.ID_TIPO_COMEX,A.QT_DIAS_TRANSITTIME_INICIAL,A.QT_DIAS_TRANSITTIME_FINAL,A.QT_DIAS_TRANSITTIME_MEDIA,A.ID_TIPO_FREQUENCIA,A.VL_FREQUENCIA,A.NM_TAXAS_INCLUDED,A.FL_ATIVO,CONVERT(varchar,A.DT_VALIDADE_FINAL,103)DT_VALIDADE_FINAL,ISNULL(A.ID_PORTO_ESCALA,0)ID_PORTO_ESCALA,ISNULL(A.ID_PORTO_ESCALA2,0)ID_PORTO_ESCALA2,ISNULL(A.ID_PORTO_ESCALA3,0)ID_PORTO_ESCALA3,ID_VIATRANSPORTE,ID_ORIGEM_PAGAMENTO
...
WHERE B.ID_TARIFARIO_FRETE_TRANSPORTADOR = " & ID)
    ...
    Private Sub dgvTaxas_RowCommand(sender As Object, e As GridViewCommandEventArgs) Handles dgvTaxas.RowCommand
        ...
        ElseIf e.CommandName = "visualizar" Then
            ...
            ds = Con.ExecutarQuery("SELECT C.ID_TABELA_FRETE_TAXA,C.ID_TIPO_ESTUFAGEM,C.ID_ITEM_DESPESA,C.ID_ORIGEM_PAGAMENTO,C.ID_BASE_CALCULO_TAXA,C.ID_MOEDA_COMPRA,C.VL_TAXA_COMPRA,C.ID_MOEDA_VENDA,C.VL_TAXA_VENDA,VL_TAXA_VENDA_MIN,ID_FRETE_TRANSPORTADOR,VL_TAXA_COMPRA_MIN, QTD_BASE_CALCULO
...
WHERE C.ID_TABELA_FRETE_TAXA = " & ID)
        ...
        Else
            ...
            If txtIDTaxa.Text = "" Then
                ...
                Else
                    ...
                    'INSERE TAXAS
                    ds = Con.ExecutarQuery("INSERT INTO TB_TABELA_FRETE_TAXA ( ID_FRETE_TRANSPORTADOR,ID_TIPO_ESTUFAGEM,ID_ITEM_DESPESA,ID_ORIGEM_PAGAMENTO,ID_BASE_CALCULO_TAXA,ID_MOEDA_COMPRA,VL_TAXA_COMPRA,ID_MOEDA_VENDA,VL_TAXA_VENDA,VL_TAXA_VENDA_MIN,VL_TAXA_COMPRA_MIN,QTD_BASE_CALCULO) VALUES (" & txtFreteTransportadorTaxa.Text & "," & ddlEstufagemTaxa.SelectedValue & " ," & ddlItemDespesa.SelectedValue & "," & ddlOrigemPagamento.SelectedValue & "," & ddlBaseCalculoTaxa.SelectedValue & "," & ddlMoedaCompra.SelectedValue & "," & txtValorTaxaCompra.Text & "," & ddlMoedaVenda.SelectedValue & "," & txtValorTaxaVenda.Text & ", " & txtValorTaxaVendaMin.Text & "," & txtValorTaxaCompraMin.Text & "," & txtQtdBaseCalculo.Text & " )")
    ...
    Sub ImportaTaxas()
        ...
        Dim Con As New Conexao_sql
        Con.Conectar()
        Dim ID_PORTO As String
        If ddlComex.SelectedValue = 1 Then
            'COMEX = 1 (IMPORTAÇÃO)
            ID_PORTO = ddlDestino.SelectedValue
        ElseIf ddlComex.SelectedValue = 2 Then
            'COMEX = 2 (EXPORTAÇÃO)
        ...
        If dsTaxas.Tables(0).Rows(0).Item("QTD") = 0 Then
            Dim ds As DataSet = Con.ExecutarQuery("SELECT A.ID_TAXA_LOCAL_TRANSPORTADOR,A.ID_ITEM_DESPESA
FROM TB_TAXA_LOCAL_TRANSPORTADOR A

INNER JOIN (
SELECT ID_ITEM_DESPESA, MAX(DT_VALIDADE_INICIAL) AS DT_VALIDADE_INICIAL,ID_BASE_CALCULO 
FROM TB_TAXA_LOCAL_TRANSPORTADOR
WHERE ID_PORTO = " & ID_PORTO & " AND ID_TRANSPORTADOR = " & ddlTransportador.SelectedValue & " AND ID_VIATRANSPORTE = " & ddlViaTransporte.SelectedValue & "  AND ID_TIPO_COMEX = " & ddlComex.SelectedValue & "

AND ID_BASE_CALCULO IN (SELECT ID_BASE_CALCULO_TAXA FROM TB_BASE_CALCULO_TAXA C WHERE C.ID_TIPO_CONTAINER IS NULL OR C.ID_TIPO_CONTAINER IN (SELECT ID_TIPO_CONTAINER FROM TB_TARIFARIO_FRETE_TRANSPORTADOR WHERE ID_FRETE_TRANSPORTADOR = " & txtID_FreteTransportador.Text & "))
...
WHERE ID_PORTO = " & ID_PORTO & " AND ID_TRANSPORTADOR = " & ddlTransportador.SelectedValue & " AND ID_VIATRANSPORTE = " & ddlViaTransporte.SelectedValue & "  AND ID_TIPO_COMEX = " & ddlComex.SelectedValue)
            If ds.Tables(0).Rows.Count > 0 Then
                ...
                For Each linha As DataRow In ds.Tables(0).Rows
                    Con.ExecutarQuery("INSERT INTO TB_TABELA_FRETE_TAXA (ID_FRETE_TRANSPORTADOR,ID_ITEM_DESPESA,VL_TAXA_COMPRA,ID_MOEDA_COMPRA,ID_BASE_CALCULO_TAXA,ID_ORIGEM_PAGAMENTO,QTD_BASE_CALCULO)   
SELECT " & txtID_FreteTransportador.Text & ", ID_ITEM_DESPESA, VL_TAXA_LOCAL_COMPRA,ID_MOEDA,ID_BASE_CALCULO,ID_ORIGEM_PAGAMENTO,QTD_BASE_CALCULO FROM TB_TAXA_LOCAL_TRANSPORTADOR WHERE ID_PORTO = " & ID_PORTO & " AND ID_TRANSPORTADOR = " & ddlTransportador.SelectedValue & " AND ID_VIATRANSPORTE = " & ddlViaTransporte.SelectedValue & " AND ID_TIPO_COMEX = " & ddlComex.SelectedValue & " AND ID_TAXA_LOCAL_TRANSPORTADOR = " & linha.Item("ID_TAXA_LOCAL_TRANSPORTADOR"))
...
AND A.ID_ITEM_DESPESA NOT IN (SELECT ID_ITEM_DESPESA FROM TB_TABELA_FRETE_TAXA WHERE ID_FRETE_TRANSPORTADOR = " & txtID_FreteTransportador.Text & "  )")
            If ds.Tables(0).Rows.Count > 0 Then
