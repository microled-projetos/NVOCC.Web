    ...
    WHERE A.ID_COTACAO = " & Request.QueryString("c"))
        If ds.Tables(0).Rows.Count > 0 Then
            ...
            ElseIf Not IsDBNull(ds.Tables(0).Rows(0).Item("CPF_CLIENTE")) Then
                lblCnpjClienteIngles.Text = ds.Tables(0).Rows(0).Item("CPF_CLIENTE")
            End If

            'INGLES

            If Not IsDBNull(ds.Tables(0).Rows(0).Item("NOME_SERVICO")) Then
                lblTituloIngles.Text = ds.Tables(0).Rows(0).Item("NOME_SERVICO") & " (" & ds.Tables(0).Rows(0).Item("NOME_ESTUFAGEM") & ")"
            End If
            ...
            If Not IsDBNull(ds.Tables(0).Rows(0).Item("NOME_CLIENTE")) Then
                lblClienteIngles.Text = ds.Tables(0).Rows(0).Item("NOME_CLIENTE").ToString
            End If

            'lblNomeIngles.Text = 
            lblDataAtualIngles.Text = Now.Date

            If Not IsDBNull(ds.Tables(0).Rows(0).Item("NR_COTACAO")) Then
                lblNumeroCotacaoIngles.Text = ds.Tables(0).Rows(0).Item("NR_COTACAO").ToString
            End If

            If Not IsDBNull(ds.Tables(0).Rows(0).Item("VL_TOTAL_PESO_BRUTO")) Then
                lblPesoBrutoIngles.Text = ds.Tables(0).Rows(0).Item("VL_TOTAL_PESO_BRUTO").ToString
            End If

            If Not IsDBNull(ds.Tables(0).Rows(0).Item("VL_TOTAL_M3")) Then
                lblM3Ingles.Text = ds.Tables(0).Rows(0).Item("VL_TOTAL_M3").ToString
            End If

            If Not IsDBNull(ds.Tables(0).Rows(0).Item("VL_PESO_TAXADO")) Then
                lblPesoTaxadoIngles.Text = ds.Tables(0).Rows(0).Item("VL_PESO_TAXADO").ToString
            End If


            If Not IsDBNull(ds.Tables(0).Rows(0).Item("PORTO_ORIGEM")) Then
                lblOrigemIngles.Text = ds.Tables(0).Rows(0).Item("PORTO_ORIGEM").ToString
            End If

            If Not IsDBNull(ds.Tables(0).Rows(0).Item("PORTO_DESTINO")) Then
                lblDestinoIngles.Text = ds.Tables(0).Rows(0).Item("PORTO_DESTINO").ToString
            End If


            If Not IsDBNull(ds.Tables(0).Rows(0).Item("VIA_ROTA")) Then
                lblViaIngles.Text = ds.Tables(0).Rows(0).Item("VIA_ROTA").ToString
            End If


            If Not IsDBNull(ds.Tables(0).Rows(0).Item("QT_TRANSITTIME_MEDIA")) Then
                lblTTimeIngles.Text = ds.Tables(0).Rows(0).Item("QT_TRANSITTIME_MEDIA").ToString & " days"
            End If


            If Not IsDBNull(ds.Tables(0).Rows(0).Item("DT_VALIDADE_COTACAO")) Then
                lblValidadeIngles.Text = ds.Tables(0).Rows(0).Item("DT_VALIDADE_COTACAO").ToString
            End If


            If Not IsDBNull(ds.Tables(0).Rows(0).Item("NOME_INCOTERM")) Then
                lblINCOTERMIngles.Text = ds.Tables(0).Rows(0).Item("NOME_INCOTERM").ToString
            End If
            ...
            ' lblTotalFreteIngles.Text = "??"

            If Not IsDBNull(ds.Tables(0).Rows(0).Item("NOME_ANALISTA")) Then
                lblAnalistaIngles.Text = ds.Tables(0).Rows(0).Item("NOME_ANALISTA").ToString
            End If
...
WHERE FL_DECLARADO = 1
    AND A.ID_COTACAO = " & Request.QueryString("c"))
        If ds.Tables(0).Rows.Count > 0 Then
            ...
            ds = Con.ExecutarQuery("SELECT CAST(SUM(A.VL_TAXA_VENDA_CALCULADO)AS varchar) + ' ' + (SELECT SIGLA_MOEDA FROM TB_MOEDA WHERE ID_MOEDA = A.ID_MOEDA_VENDA)as  descricao  FROM TB_COTACAO_TAXA A  WHERE ID_COTACAO = " & Request.QueryString("c") & " AND FL_DECLARADO = 1 GROUP BY A.ID_MOEDA_VENDA ")
            If ds.Tables(0).Rows.Count > 0 Then
            ...
            lblTotalTaxasOrigemIngles.Text = DescTotalOrigem
...
WHERE FL_DECLARADO = 0
    AND A.ID_COTACAO = " & Request.QueryString("c"))
        If ds.Tables(0).Rows.Count > 0 Then
            ...
            ' lblTaxasDestinoCalcIngles.Text = taxasDestinoCalc


            'total destino
            ds = Con.ExecutarQuery("SELECT CAST(SUM(A.VL_TAXA_VENDA_CALCULADO)AS varchar) + ' ' + (SELECT SIGLA_MOEDA FROM TB_MOEDA WHERE ID_MOEDA = A.ID_MOEDA_VENDA)as  descricao  FROM TB_COTACAO_TAXA A  WHERE ID_COTACAO = " & Request.QueryString("c") & " AND FL_DECLARADO = 0 GROUP BY A.ID_MOEDA_VENDA ")
            If ds.Tables(0).Rows.Count > 0 Then
            ...
            lblTotalTaxasDestinoIngles.Text = DescTotalDestino
        ...
        ds = Con.ExecutarQuery("SELECT CAST(SUM(ISNULL(A.VL_TAXA_VENDA_CALCULADO,0))AS varchar) + ' ' + (SELECT SIGLA_MOEDA FROM TB_MOEDA WHERE ID_MOEDA = A.ID_MOEDA_VENDA)as  descricao  FROM TB_COTACAO_TAXA A  WHERE ID_COTACAO = " & Request.QueryString("c") & "  GROUP BY A.ID_MOEDA_VENDA ")
        If ds.Tables(0).Rows.Count > 0 Then
        ...
        'lblTotalBRLIngles.Text = ds.Tables(0).Rows(0).Item("FL_ACESSAR")


        'lblTaxasOrigemIngles.Text = lblTaxasOrigemIngles.Text.Replace("³", "<sup>3</sup>")
        'lblTaxasDestinoIngles.Text = lblTaxasDestinoIngles.Text.Replace("³", "<sup>3</sup>")
        'lblfretesIngles.Text = lblfretesIngles.Text.Replace("³", "<sup>3</sup>")
    ...
    Sub CONTAINER()
        Dim Con As New Conexao_sql
        Con.Conectar()
        Dim ds As DataSet = Con.ExecutarQuery("SELECT NM_TIPO_CONTAINER, CAST((VL_FRETE_VENDA/QT_CONTAINER)AS NUMERIC(13,2))VALOR,SUM(QT_CONTAINER)QTD
