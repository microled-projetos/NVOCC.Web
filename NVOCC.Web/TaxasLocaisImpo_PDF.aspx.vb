Public Class TaxasLocaisImpo_PDF
...
from TB_TAXA_LOCAL_TRANSPORTADOR A WHERE ID_TIPO_COMEX = 1 ORDER BY ID_TRANSPORTADOR")
...
from TB_TAXA_LOCAL_TRANSPORTADOR A WHERE ID_TIPO_COMEX = 1 and ID_TRANSPORTADOR = " & linhaTitulo.Item("ID_TRANSPORTADOR") & " ORDER BY ID_PORTO")
            For Each linha1 As DataRow In ds1.Tables(0).Rows
                tabela = "<div Class='porto panel panel-default'><div class='panel-heading'><div class='panel-title' style='text-align:center;background-color:#bddea0'>" & linha1.Item("NM_PORTO").ToString() & "</div></div><table border='1' class='subtotal table table-bordered'>"
...
Left Join TB_ITEM_DESPESA B ON A.ID_ITEM_DESPESA = B.ID_ITEM_DESPESA  WHERE ID_PORTO = " & ID_PORTO & " AND ID_TRANSPORTADOR = " & linhaTitulo.Item("ID_TRANSPORTADOR") & " and ID_TIPO_COMEX = 1  ")
