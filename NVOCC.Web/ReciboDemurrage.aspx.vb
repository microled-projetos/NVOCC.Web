...
GROUP BY ID_PARCEIRO_IMPORTADOR,NR_BL,NR_PROCESSO,ID_BL_MASTER,A.DT_LANCAMENTO,A.NR_RECIBO")
                If ds.Tables(0).Rows.Count > 0 Then
                    ...
                    Else
                        Dim numero As String = NumeracaoDoc.Numerar(4)
                        lblNumeroRecibo.Text = numero
                        Con.ExecutarQuery("UPDATE [dbo].[TB_DEMURRAGE_FATURA] SET NR_RECIBO = '" & numero & "' WHERE ID_DEMURRAGE_FATURA =" & ID)
                        Con.ExecutarQuery("UPDATE [dbo].[TB_NUMERACAO] SET NR_RECIBO_DEMURRAGE = '" & numero & "' WHERE ID_NUMERACAO = 5")
                        Con.Fechar()
                    End If
