...
namespace ABAINFRA.Web
{
    ...
    public class DemurrageService : System.Web.Services.WebService
    {
        ...
        public string imprimirDemonstrativoRateio(string blmaster)
        {
            ...
            SQL += "AND A.ID_BL_MASTER = '"+blmaster+ "' AND ISNULL(E.TP_EXPORTACAO ,'') = '' ";
        ...
        public string imprimirDemonstrativoRateioTotal(string blmaster)
        {
            ...
            SQL += "SUM((CASE WHEN C.FL_ABATER_ISS = 1 THEN C.VL_LIQUIDO - C.VL_ISS ELSE C.VL_LIQUIDO END)+(CASE WHEN C.FL_ABATER_ISS = 1 THEN C.VL_ISS ELSE 0 END)) AS RATEIO_TOTAL, ";
            SQL += "SUM((CASE WHEN C.FL_ABATER_ISS = 1 THEN C.VL_ISS ELSE 0 END)) AS RATEIOISS, ";
            SQL += "SUM((CASE WHEN C.FL_ABATER_ISS = 1 THEN C.VL_LIQUIDO ELSE 0 END)) AS NFLIQUIDO, ";
            SQL += "SUM((CASE WHEN C.FL_ABATER_ISS = 1 THEN C.VL_ISS ELSE 0 END)+(CASE WHEN C.FL_ABATER_ISS = 1 THEN C.VL_LIQUIDO ELSE 0 END)) AS RATEIONF ";
            SQL += "FROM TB_BL A ";
            SQL += "INNER JOIN TB_BL B ON A.ID_BL_MASTER = B.ID_BL ";
            SQL += "INNER JOIN TB_CONTA_PAGAR_RECEBER_ITENS C ON A.ID_BL = C.ID_BL ";
            SQL += "INNER JOIN TB_CONTA_PAGAR_RECEBER E ON C.ID_CONTA_PAGAR_RECEBER = E.ID_CONTA_PAGAR_RECEBER ";
